    ...
    public class TestClient : GridClient
    {
        ...
        void Self_IM(object sender, InstantMessageEventArgs e)
        {
            ...
            if (e.IM.FromAgentID == MasterKey || (GroupCommands && groupIM))
            {
                ...
                Jenny.Console.WriteLine(String.Format("<{0} ({1})> {2}: {3} (@{4}:{5})", e.IM.GroupIM ? "GroupIM" : "IM", e.IM.Dialog, e.IM.FromAgentName, e.IM.Message,
                    e.IM.RegionID, e.IM.Position));

                if (e.IM.Dialog == InstantMessageDialog.RequestTeleport)
                {
                    Jenny.Console.WriteLine("Accepting teleport lure.");
            ...
            else
            {
                ...
                Jenny.Console.WriteLine(String.Format("<{0} ({1})> {2} (not master): {3} (@{4}:{5})", e.IM.GroupIM ? "GroupIM" : "IM", e.IM.Dialog, e.IM.FromAgentName, e.IM.Message,
                    e.IM.RegionID, e.IM.Position));
                return;
            }
        }
        ...
        public void RegisterAllCommands(Assembly assembly)
        {
            foreach (Type t in assembly.GetTypes())
            {
                ...
                {
                    Jenny.Console.WriteLine(e.ToString());
                }
            }
        }
        ...
        private void AlertMessageHandler(object sender, PacketReceivedEventArgs e)
        {
            ...
            Debug.Log(String.Format("[AlertMessage] " + Utils.BytesToString(message.AlertData.Message), Helpers.LogLevel.Info, this));
        }
