...
namespace OpenMetaverse.TestClient_
{
    public class ImportCommand : Command
    {
        ...
        public override string Execute(string[] args, UUID fromAgentID)
        {
            ...
            Jenny.Console.WriteLine("Importing " + linksets.Count + " structures.");
            ...
            foreach (Linkset linkset in linksets.Values)
            {
                if (linkset.RootPrim.LocalID != 0)
                {
                    ...
                    if (linkset.Children.Count != 0)
                    {
                        ...
                        if (primDone.WaitOne(1000 * linkset.Children.Count, false))
                            ...
                            Jenny.Console.WriteLine(String.Format("Warning: Failed to link {0} prims", linkQueue.Count));
                    }
                    else
                    {
                        Client.Objects.SetRotation(Client.Network.CurrentSim, rootLocalID, rootRotation);
                ...
                else
                {
                    ...
                    Jenny.Console.WriteLine("WARNING: Skipping a linkset with a missing root prim");
                }
        ...
        void Objects_OnNewPrim(object sender, PrimEventArgs e)
        {
            ...
            switch (state)
            {
                ...
                case ImporterState.RezzingChildren:
                    ...
                    {
                        Jenny.Console.WriteLine("Setting properties for " + prim.LocalID);
