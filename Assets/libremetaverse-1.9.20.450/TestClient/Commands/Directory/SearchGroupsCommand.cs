    ...
    class SearchGroupsCommand : Command
    {
        ...
        void Directory_DirGroups(object sender, DirGroupsReplyEventArgs e)
        {
            if (e.MatchedGroups.Count > 0)
            {
                ...
                {
                    Jenny.Console.WriteLine(String.Format("Group {1} ({0}) has {2} members", group.GroupID, group.GroupName, group.Members));
                }
            }
            else
            {
                Jenny.Console.WriteLine("Didn't find any groups that matched your query :(");
            }
            waitQuery.Set();
        }        
    }
