    ...
    public class JoinGroupCommand : Command
    {
        ...
        void Directory_DirGroups(object sender, DirGroupsReplyEventArgs e)
        {
            if (queryID == e.QueryID)
            {
                ...
                {
                    Jenny.Console.WriteLine("ERROR: Got an empty reply");
                }
                else
                {
                    if (e.MatchedGroups.Count > 1)
                    {
                         ...
                         */
                        Jenny.Console.WriteLine("Matching groups are:\n");
                        foreach (DirectoryManager.GroupSearchData groupRetrieved in e.MatchedGroups)
                        {
                            Jenny.Console.WriteLine(groupRetrieved.GroupName + "\t\t\t(" +
        ...
        {
            Jenny.Console.WriteLine(Client + (e.Success ? " joined " : " failed to join ") + e.GroupID);
            ...
            if (success)
            {
                Jenny.Console.WriteLine(Client.ToString() + " setting " + groupID.ToString() + " as the active group");
