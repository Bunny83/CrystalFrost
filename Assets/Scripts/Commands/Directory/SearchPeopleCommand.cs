    ...
    class SearchPeopleCommand : Command
    {
        ...
        void Directory_DirPeople(object sender, DirPeopleReplyEventArgs e)
        {
            if (e.MatchedPeople.Count > 0)
            {
                ...
                {
                    Jenny.Console.WriteLine(String.Format("{0} {1} ({2})", agent.FirstName, agent.LastName, agent.AgentID));                   
                }
            }
            else
            {
                Jenny.Console.WriteLine("Didn't find any people that matched your query :(");
            }
            waitQuery.Set();
        }
    }
