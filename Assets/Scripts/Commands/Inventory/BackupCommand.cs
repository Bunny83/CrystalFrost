...
namespace OpenMetaverse.TestClient_
{
    ...
    public class BackupCommand : Command
    {
        ...
        {
            QueueWorker = null;
            Jenny.Console.WriteLine(BackgroundBackupStatus);
        }
        ...
        {
            Jenny.Console.WriteLine(Name + ": Inventory walking thread done.");
            BackupWorker = null;
        }
        ...
        private void Assets_OnAssetReceived(AssetDownload asset, Asset blah)
        {
            lock (CurrentDownloads)
            {
                ...
                if (r != null && r.AssetID == asset.AssetID)
                {
                    ...
                    {
                        TextItemErrors++;
                        Jenny.Console.WriteLine(String.Format("{0}: Download of asset {1} ({2}) failed with status {3}", Name, r.FileName,
