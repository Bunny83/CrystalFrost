...
namespace OpenMetaverse.TestClient_
{
    public class ImGroupCommand : Command
    {
        ...
        public override string Execute(string[] args, UUID fromAgentID)
        {
            ...
            if (UUID.TryParse(args[0], out ToGroupID))
            {
                ...
                if (message.Length > 1023)
                {
                    ...
                    Jenny.Console.WriteLine("Message truncated at 1024 characters");
                }
        ...
        void Self_GroupChatJoined(object sender, GroupChatJoinedEventArgs e)
        {
            if (e.Success)
            {
                Jenny.Console.WriteLine(String.Format("Joined {0} Group Chat Success!", e.SessionName));
                WaitForSessionStart.Set();
            }
            else
            {
                Jenny.Console.WriteLine("Join Group Chat failed :(");
            }
        }
